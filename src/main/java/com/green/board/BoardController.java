...
/*
    ...
    1. 쿼리스티링 방식 (파라미터라고 부르기도 함), url에 데이터를 포함하는 방식
    ...
    예를 들어 name은 홍길동, age는 22살, height는 178.2 데이터를 JSON으로 표현을 하면
    ...
    이렇게 표현하는 문자열이다.
    {}는 객체를 의미하고 []는 배열을 의미한다. "" - 문자열, 숫자형은 ""없이 표현한다.
    Key는 무조건 ""감싸줘야 한다.
    ...
    일단 데이터가 있었을 때는 get방식이 처리 속도가 빠름, 데이터 처리가 아닌 단순 화면을 띄울 때도
    get방식을 사용함
    ...
    (Rendering) 화면마다 데이터가 필요하면 BE 요청을 한다.
    누가 ? FE작업코드가 요청을 보낸다.
    ...
    (get) / board?aaa=2 - /board?page=1 과 같은 요청이다. URL이 같으면 같은 요청
